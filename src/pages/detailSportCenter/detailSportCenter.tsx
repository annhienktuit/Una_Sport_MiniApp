...
import { openChatScreen, slectedSportCenterState } from "../../state";
...
const DetailSportCenter: React.FunctionComponent = () => {
  ...
  function getYards() {
    ...
    const yardPromises = yardIds.map((yardId) =>
      get(child(dbRef, `Yard/${yardId}`))
    );
    ...
    Promise.all(yardPromises)
      .then((yardSnapshots) => {
        ...
        const yards = yardSnapshots.map((snapshot) =>
  ...
  function formatTimeRange(start: number, end: number): string {
    ...
    const startTime = new Date(start * 1000).toLocaleTimeString(
      "en-US",
      formatOptions
    );
    const endTime = new Date(end * 1000).toLocaleTimeString(
      "en-US",
      formatOptions
    );
  ...
  const handleClick = (data: Yard) => {
    ...
    setBottomSheetText(
      generateBookingInfo(data, sportCenter!, date, timeRange)
    );
  };
          ...
          <div className="mr-4 ml-4 flex-col flex">
            ...
            <div className="bg-button p-[16px] flex rounded-[100px] items-center justify-center text-buttonText">
              <Text
                ...
                size="normal"
                onClick={() => {
                  openChatScreen("3017453454363623756");
                }}
              >
                Liên hệ Zalo
              </Text>
